...
2. Read `AGENTS.md/CLAUDE.md` — extract:
...
|-----------|------|--------|--------|
| [constraint] | HARD/SOFT | Brief/AGENTS.md/CLAUDE.md/Implicit | Validated/Flagged |
...
- Implicit constraints not in the brief but evident from AGENTS.md/CLAUDE.md patterns
...
**Numeric HARD constraints — empirical evidence required:** Every numeric HARD constraint must cite evidence from the most representative executable path available, using the constraint's unit (ms, MB/GB, RPS, %/p99, $/call, count). Single-component benchmarks do not prove end-to-end budgets. If unmeasured or over budget, mark provisional and add a reconciliation gate before build/ship.
...
- Project patterns confirmed in AGENTS.md/CLAUDE.md/skills
...
- **Pattern alignment:** how it fits project conventions — cite the specific AGENTS.md/CLAUDE.md section or skill
...
Before exiting design, list what is IN scope for the first ship and what is OUT of scope (deferred to v2). One-line rationale per deferred item. The MVP boundary is normative and supersedes later-cycle additions.
...
|------------|-------------------|
| Pattern recommendation | Cite AGENTS.md/CLAUDE.md section or project skill by name |
...
**Pseudocode primitives:** Reject ungrounded pseudocode before /team-plan; apply the Grounding Rules row above.
...
**SOT consolidation on re-entry:** After a /team-review MUST-FIX, update the design body to reflect the resolution OR mark superseded text stale. Do not proceed to /team-plan while a resolution table contradicts unchanged body prose.
...
**Read:**
- `AGENTS.md/CLAUDE.md` — always (project context, conventions, guardrails)
