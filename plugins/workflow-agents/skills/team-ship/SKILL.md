...
Run the full test suite (test command from AGENTS.md/CLAUDE.md). Apply the cross-cutting `team-verification-before-completion` protocol — fresh test output, read directly, before any "ready to ship" claim. A green run from yesterday is not evidence; it has to be from this branch state, this session.
...
**READ:** AGENTS.md/CLAUDE.md (test command, branch conventions), git status, git log.
