...
Returned to the lead when the worker completes (see the skill's § Dispatch by Runtime for how workers report back on this runtime).
...
[If an interpretation call]: Use [X] because [reason from AGENTS.md/CLAUDE.md / plan]
...
- [ ] Only after all checks pass: mark the builder's task complete (see the skill's § Dispatch by Runtime for the per-runtime status-update primitive)
