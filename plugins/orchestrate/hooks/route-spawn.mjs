...
// role is never rewritten and never costs a picker call. Any classifier failure
// or abstention adds no model or effort; existing caps remain.
...
try {
  ...
  // A named role keeps its installed model intent: no pick, no rewrite.
  const named = lib.isNamedRole(input.subagent_type);
  // Only ask when something is missing; an explicit model + shim needs caps alone.
  const needsPick = !named && (!input.model || lib.isRoleless(input.subagent_type));
  ...
  lib.logDispatch({
    ...
    ...(picked ?? { decision: named ? 'named-role' : 'explicit' }),
  });
  if (updated) {
